...
const { getEffectivePlanSize, validatePlanSetback } = require("./plan");
...
function drawLegend(canvas, originX, originY, legend, contentWidth) {
  const totalWidth = (legend.length + 1) * LEGEND_ITEM_STEP;
  let x = originX + (contentWidth - totalWidth) / 2;
...
function renderSketch({ rows, title, styles, plan, elevation, subtitle }) {
  ...
  if (legend.length > 0) {
    drawLegend(canvas, originX, legendY, legend, mainContentWidth);
  }
