...
const { getEffectivePlanSize, validatePlanSetback, isPlotLimitedCell } = require("./plan");

const CELL_SIZE = 22;
const HEADER_OFFSET = 72;
const ELEV_GAP = 20;
const LEGEND_GAP = 16;
const LEGEND_ITEM_STEP = CELL_SIZE + 88;
const LEGEND_LABEL_GAP = LEGEND_ITEM_STEP - CELL_SIZE;
/** Opacity for painted cells in setback / outside the plot. */
const PLOT_LIMITED_OPACITY = 0.6;
/** Gap between the outer frame and the grid so edge cell strokes stay visible. */
const OUTER_FRAME_GAP = 5;
...
function drawStyledCell(canvas, x, y, style, options = {}) {
  const pad = 1.5;
  const opacityScale = options.plotLimited ? PLOT_LIMITED_OPACITY : 1;

  if (style.transparent && !options.forceBorder) {
    if (options.plotLimited) {
      canvas.fillRect(x + pad, y + pad, CELL_SIZE - pad * 2, CELL_SIZE - pad * 2, "#fdecea", 0.35);
      canvas.sketchRect(x, y, CELL_SIZE, CELL_SIZE, {
        stroke: style.stroke ?? "#dddddd",
        strokeWidth: 1.2,
        roughness: 1,
        bowing: 0.4,
        strokeOpacity: PLOT_LIMITED_OPACITY,
      });
    }
    return;
  }
  ...
  if (!style.transparent || options.forceBorder) {
    canvas.sketchRect(x, y, CELL_SIZE, CELL_SIZE, {
      ...
      bowing: 0.4,
      ...(opacityScale < 1 ? { strokeOpacity: opacityScale } : {}),
...
function drawPlanUnderlay(canvas, originX, originY, plan, gridCols, gridRows) {
  ...
  const { width: plotW, depth: plotD } = getEffectivePlanSize(width, depth, gridCols, gridRows);
  const setbackError = validatePlanSetback(setback, plotW, plotD);
  if (setbackError) {
  ...
  canvas.sketchRect(
    originX - OUTER_FRAME_GAP,
    originY - OUTER_FRAME_GAP,
    plotW * CELL_SIZE + OUTER_FRAME_GAP * 2,
    plotD * CELL_SIZE + OUTER_FRAME_GAP * 2,
    {
      stroke: "#888888",
      strokeWidth: 2,
      roughness: 1,
...
function drawElevationColumn(canvas, originX, originY, elevation, alignBottomY) {
  ...
  canvas.sketchRect(
    originX - OUTER_FRAME_GAP,
    columnTop - OUTER_FRAME_GAP,
    CELL_SIZE + OUTER_FRAME_GAP * 2,
    displayRows * CELL_SIZE + OUTER_FRAME_GAP * 2,
    {
      stroke: "#888888",
      strokeWidth: 1.8,
      roughness: 1,
...
function legendRowWidth(legend) {
  const itemCount = legend.length + 1;
  return itemCount * CELL_SIZE + (itemCount - 1) * LEGEND_LABEL_GAP;
}

function drawLegend(canvas, centerX, originY, legend) {
  const totalWidth = legendRowWidth(legend);
...
function renderSketch({ rows, title, styles, plan, elevation, subtitle }) {
  ...
  for (let row = 0; row < gridRows; row++) {
    for (let col = 0; col < gridCols; col++) {
      ...
      const { x, y } = cellOrigin(originX, originY, col, row);
      const plotLimited = plan?.enabled && isPlotLimitedCell(row, col, plan, gridCols, gridRows);
      drawStyledCell(canvas, x, y, style, { plotLimited });
    }
  }
  ...
  canvas.sketchRect(
    originX - OUTER_FRAME_GAP,
    originY - OUTER_FRAME_GAP,
    gridCols * CELL_SIZE + OUTER_FRAME_GAP * 2,
    gridRows * CELL_SIZE + OUTER_FRAME_GAP * 2,
    {
      stroke: "#888888",
      strokeWidth: 1.8,
      roughness: 1,
  ...
  const mainContentWidth =
    gridCols * CELL_SIZE + (elevation?.enabled ? ELEV_GAP + CELL_SIZE : 0);
  const diagramCenterX = originX + mainContentWidth / 2;

  const legend = legendForGrid(rows, styles);
  const legendY = contentBottom + LEGEND_GAP;
  if (legend.length > 0) {
    drawLegend(canvas, diagramCenterX, legendY, legend);
  }
  ...
  canvas.drawHeader(title, parts.join(" · "), diagramCenterX);
  return canvas.toString({ diagramWidth: mainContentWidth });
}
